...
import type { AnalyticsContent, AnalyticsTopic } from './types';
...
export const init: ModuleController['init'] = async (ctx: ModuleContext) => {
  // The legacy modal is heavily DOM-id-coupled (it expects #analytics-engineer-modal,
  // #card-indicators, #prev-card-btn, #next-card-btn). For the v2 lifecycle we
  // simply delegate to the legacy showAnalyticsModal so the existing behaviour
  // is preserved. The shell is responsible for making sure the modal chrome
  // exists when init() is called.
  const active = new Date(`${ctx.today}T00:00:00`);
  const referenceDate = Number.isNaN(active.getTime()) ? new Date() : active;
  await showAnalyticsModal(referenceDate);
};
...
export const AnalyticsModule: ModuleController = { init, destroy };
