...
export default defineSchema({
  ...
  tasks: defineTable({
    ...
    reminderRevision: v.optional(v.string()),
  ...
  // A user may install the PWA on several devices. Each device owns one Web
  // Push subscription; reminder delivery fans out to every active endpoint.
  todoPushSubscriptions: defineTable({
    userId: v.id('users'),
    endpoint: v.string(),
    p256dh: v.string(),
    auth: v.string(),
